...
def init_student_groups():
    ...
    list_of_student_groups = [
        student_group1, student_group2, student_group3, student_group4
    ]
...
def main():
    ...
    file_path_past_venti = r"./past-venti.json"
    ...
    with open(file_path_past_venti, "r") as past_venti_file:
        try:
            ...
            file_path_initial_groups = r"./venti-initial-student-groups.json"
            ...
            with open(file_path_initial_groups,
                      "r") as initial_student_groups_file:
                # もとのグループを読み込む
        ...
        # past venti が空の場合
            ...
            file_location = r"./venti-initial-student-groups.json"
            ...
            with open(file_location, "w") as initial_student_groups_file:
                json.dump(initial_student_groups,
                          initial_student_groups_file,
                          indent=4)
    ...
    # すべての組み合わせを網羅していた場合
    if len(past_venti_list) == number_of_combinations:
        print("すべての組み合わせを網羅しました。")

    # まだ可能な組み合わせが残っている場合`
    else:
        # 前回までのグループのリストを読み込む（venti に一度は入った人が削除されているリスト）
        file_path_current_student = r"./current-student-groups.json"
        ...
        with open(file_path_current_student,
                  "r") as current_student_groups_file:
            try:
                current_student_groups = json.load(current_student_groups_file)
                # ID : Name になっているのを、ID だけにする
            except ValueError:
                # 空の場合は、空のリストにする
                current_student_groups = [[], [], [], []]

        # もし、グループリストに誰もいなくなっていたら、グループをリセット
        # （= 全員一度 venti のグループに入った）
        # group0_list = current_student_groups.get("group0")
        if len(current_student_groups[0]) == 0:
            # 本来は再びもとのリストを取得
            current_student_groups = initial_student_groups

        # venti を作る！
        (venti1, current_student_groups,
         past_venti_list) = create_venti(current_student_groups,
                                         past_venti_list)
        print("Venti1:")
        print(venti1)
        print()
        (venti2, current_student_groups,
         past_venti_list) = create_venti(current_student_groups,
                                         past_venti_list)
        print("Venti2:")
        print(venti2)
        ...
        # TODO: ID : Name に戻す
        with open(file_path_current_student,
                  "w") as current_student_groups_file:
            json.dump(current_student_groups,
                      current_student_groups_file,
                      indent=4)
